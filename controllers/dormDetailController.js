...
const set7Days = (c, data) => {
    ...
    for (let i = Object.entries(exData).length - 6; i <= Object.entries(exData).length; i++) {
...
const set30Days = (c, data) => {
    ...
    for (let i = Object.entries(exData).length - 29; i <= Object.entries(exData).length; i++) {
        let val = exData["Day" + i]
        let sum = window.constants.utils.getDailySummary(val)
        res.push(cEnum.getColorValue(sum[0]))
        totals["red"] += sum[1][0]
...
const set60Days = (c, data) => {
    ...
    for (let i = Object.entries(exData).length - 59; i <= Object.entries(exData).length; i++) {
        let val = exData["Day" + i]
        let sum = window.constants.utils.getDailySummary(val)
        res.push(cEnum.getColorValue(sum[0]))
        totals["red"] += sum[1][0]
...
window.ipcComms.dormDetailViewID((id) => {
    ...
    for (let i = 0; i < id.students.length; i++) {
        ...
        student.className = "col-lg ml-4 text-dark fw-bold btn btn-link"
